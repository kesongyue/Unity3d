...
public class GenGameObjects : MonoBehaviour, SceneController
{
    ...
    int PriestsOnStartNumbers = 3;
    int DevilsOnStartNumbers = 3;
    int PriestsOnEndNumbers = 0;
    int DevilsOnEndNumbers = 0;
    int PriestsOnBoatNumbers = 0;
    int DevilsOnBoatNumbers = 0;

    GameObject shoreLeft;
    GameObject shoreRight;
    int boatSign =1;  //1 -->Left -1 --> Right
    int boat_capacity;
    Vector3 shoreLeftPosition = new Vector3(-8, 1, -8f);
    Vector3 shoreRightPosition = new Vector3(-8, 1, 8f);
    public Vector3 boatPositionLeft = new Vector3(-8, 0.5f, -4);
    public Vector3 boatPositionRight = new Vector3(-8, 0.5f, 4);
    Vector3[] positionOfObjectOnBoat=new Vector3[2] { new Vector3(0, 0.8f, -0.3f), new Vector3(0, 0.8f, 0.3f) };

    Vector3[] RolePositions = new Vector3[] {new Vector3(-8,1.8f,-5.5f), new Vector3(-8,1.8f,-6f), new Vector3(-8,1.8f,-6.5f),
                new Vector3(-8,1.8f,-7f), new Vector3(-8,1.8f,-8f), new Vector3(-8,1.8f,-9f)};
    GameObject boat;
    GameObject[] devils = new GameObject[3];
    GameObject[] priests = new GameObject[3];
